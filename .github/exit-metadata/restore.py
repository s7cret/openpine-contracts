"""Reconstruct exact reviewed commits and preserve release/branch identities."""
...
STAGING = 'ops/rc6-exit-metadata-20260906'
REPOS = {'s7cret/openpine', 's7cret/backtest_engine', 's7cret/openpine-contracts'}
...
def restore(path,evidence):
    ...
    for item,patch in zip(plan['commits'],patches,strict=True):
        ...
        if item.get('regenerate') is not None:
            if item['regenerate']!='contracts' or plan['repository']!='s7cret/openpine-contracts':
                raise ValueError('unapproved generator')
            subprocess.run([sys.executable,'scripts/generate_schemas.py'],check=True)
            git('add','openpine_contracts/schemas')
